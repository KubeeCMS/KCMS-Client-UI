...
class BBP_Members extends Widget_Base {
	...
	protected function _register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .diy-members-list__item' => 'margin-bottom: {{SIZE}}px',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors'  => [
					'{{WRAPPER}} .diy-members' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .diy-block-header__extra a' => 'color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'type'      => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .diy-members div.item-options' => 'border-bottom-color: {{VALUE}}',
				],
			]
		);
		...
		$this->add_group_control(
			...
			[
				...
				'placeholder' => '1px',
				'default'     => '1px',
				'selector'    => '{{WRAPPER}} .diy-members-list__avatar img',
				'separator'   => 'before',
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'size_units' => [ 'px', '%' ],
				'selectors'  => [
					'{{WRAPPER}} .diy-members-list__avatar img' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}}; overflow: hidden;',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .diy-members-list__avatar img' => 'opacity: {{SIZE}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .diy-members-list--align-left .diy-members-list__item .diy-members-list__avatar'  => 'margin-right: {{SIZE}}{{UNIT}}',
					'{{WRAPPER}} .diy-members-list--align-righ .diy-members-list__item .diy-members-list__avatar'  => 'margin-left: {{SIZE}}{{UNIT}}',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'default'   => '#122B46',
				'selectors' => [
					'{{WRAPPER}} .diy-members-list__name a' => 'color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} .diy-members-list__name a',
	...
	protected function render() {
		...
		<div class="diy-members">

			<div class="diy-block-header flex align-items-center">
				<div class="diy-block-header__title"><h3><?php _e( 'Members', 'buddyboss-theme' ); ?></h3></div>
				<?php if ($settings['switch_more']) : ?>
					<div class="diy-block-header__extra push-right">
			...
			<?php if ($settings['switch_filter']) : ?>
				<div class="item-options border-<?php echo $settings['filter_border_style']; ?>">
					<?php foreach ( $members_type as $mtype ) { ?>
						<a href="#" id="diy-<?php echo $mtype; ?>-members" class="diy-members__tab <?php echo $mtype == $type ? 'selected' : ''; ?>" data-type="<?php echo $mtype; ?>"><?php echo $mtype; ?></a>
					<?php } ?>
				</div>
			<?php endif; ?>
			...
			<div class="bbel-list-flow">
				<?php foreach ( $members_type as $mtype ) {
					...
					if ( bp_has_members( $members_args ) ) : ?>
						...
						<div class="diy-members-list diy-members-list--<?php echo $mtype; ?> diy-members-list--align-<?php echo esc_attr($settings['alignment']); ?> <?php echo $mtype == $type ? 'active' : ''; ?>">

							<?php $this->add_render_attribute('diy-member', 'class', 'diy-members-list__item'); ?>

							<?php while ( bp_members() ) : bp_the_member(); ?>

								<div <?php echo $this->get_render_attribute_string('diy-member'); ?>>
									<?php if ($settings['switch_avatar']) : ?>
										<div class="diy-members-list__avatar" <?php if ( $settings['switch_tooltips'] == 'yes' ) { ?>data-balloon-pos="<?php echo ( $settings['alignment'] == 'left' ) ? 'right' : 'left'; ?>" data-balloon="<?php echo bp_get_member_last_active(); ?>"<?php } ?>>
									...
									<?php if ($settings['switch_name']) : ?>
										<div class="diy-members-list__name fn"><a href="<?php bp_member_permalink(); ?>"><?php bp_member_name(); ?></a></div>
									<?php endif; ?>	
									<?php
