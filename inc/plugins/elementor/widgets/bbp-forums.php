...
class BBP_Forums extends Widget_Base {
	...
	protected function _register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .diy-forums__list > li' => 'margin-bottom: {{SIZE}}px;padding-bottom: {{SIZE}}px',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors'  => [
					'{{WRAPPER}} .diy-forums' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .diy-forums__avatar' => 'flex: 0 0 {{SIZE}}px;',
					'{{WRAPPER}} .diy-forums__avatar img.avatar' => 'width: {{SIZE}}px;',
					'{{WRAPPER}} .diy-forums__avatar img.avatar' => 'max-width: {{SIZE}}px;',
				],
			]
		);
		...
		$this->add_group_control(
			...
			[
				...
				'placeholder' => '1px',
				'default'     => '1px',
				'selector'    => '{{WRAPPER}} .diy-forums__avatar img.avatar',
				'separator'   => 'before',
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'size_units' => [ 'px', '%' ],
				'selectors'  => [
					'{{WRAPPER}} .diy-forums__avatar img.avatar' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}}; overflow: hidden;',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .diy-forums__avatar img.avatar' => 'opacity: {{SIZE}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .diy-forums__avatar'  => 'margin-right: {{SIZE}}{{UNIT}}',
				],
			]
		);
		...
		$this->add_control(
			...
			array(
				...
				'selectors' => array(
					...
					'{{WRAPPER}} .diy-forums__ww .bs-replied a' => 'color: {{VALUE}}',
				),
			)
		);
		...
		$this->add_control(
			...
			array(
				...
				'selectors' => array(
					...
					'{{WRAPPER}} .diy-forums__ww .bs-replied a:hover' => 'color: {{VALUE}}',
				),
			)
		);
		...
		$this->add_group_control(
			...
			array(
				...
				'selector' => '{{WRAPPER}} .item-meta > div,{{WRAPPER}} .diy-forums__item .diy-forums__ww .bs-replied > a.bbp-author-link span',
			)
		);
	...
	protected function render() {
		...
		<div class="diy-forums <?php echo ( !bbp_has_topics( $args ) ) ? 'diy-forums--blank' : ''; ?>">

			<?php if ( bbp_has_topics( $args ) ) : ?>

				<div class="diy-block-header flex align-items-center">
					<div class="diy-block-header__title"><h3><?php _e( 'Forums', 'buddyboss-theme' ); ?></h3></div>
				...
				<div class="bbel-list-flow">
					<ul class="diy-forums__list bbp-topics1 bs-item-list bs-forums-items list-view">

						<?php while ( bbp_topics() ) : bbp_the_topic(); ?>

							<li>
								<?php $class = bbp_is_topic_open() ? '' : 'closed'; ?>
								<div class="diy-forums__item">
									
									<div class="flex">
										<?php if ($settings['switch_avatar']) : ?>
											<div class="diy-forums__avatar">
										...
										<div class="item">
											...
											<div class="item-meta bb-reply-meta">
												...
												<div class="diy-forums__last-reply">
													<?php
			...
			<?php else : ?>
				...
				<div class="diy-no-data diy-no-data--forums">
					<img class="diy-no-data__image" src="<?php echo get_template_directory_uri(); ?>/assets/images/svg/dfy-no-data-icon02.svg" alt="Forums" />
					<?php bbp_get_template_part( 'feedback', 'no-topics' ); ?>
					<a href="<?php echo home_url(bbp_get_root_slug()); ?>" class="diy-no-data__link"><?php _e( 'Start a Discussion', 'buddyboss-theme' ); ?></a>
				</div>
