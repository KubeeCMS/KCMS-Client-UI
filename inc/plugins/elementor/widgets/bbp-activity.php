...
class BBP_Activity extends Widget_Base {
	...
	protected function _register_controls() {
		...
		$this->add_control(
			...
			array(
				...
				'selectors'  => array(
					'{{WRAPPER}} .diy-activity' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				),
			)
		);
		...
		$this->add_control(
			...
			array(
				...
				'selectors' => array(
					'{{WRAPPER}} .diy-activity .activity-list.item-list .activity-item .activity-avatar' => 'flex: 0 0 {{SIZE}}px;',
					'{{WRAPPER}} .diy-activity .activity-list .activity-item div.item-avatar img' => 'max-width: {{SIZE}}px;',
				),
			)
		);
		...
		$this->add_control(
			...
			array(
				...
				'selectors' => array(
					'{{WRAPPER}} .diy-activity .activity-list.item-list .activity-item .activity-avatar' => 'margin-right: {{SIZE}}{{UNIT}}',
				),
			)
		);
	...
	protected function render() {
		...
		<div class="diy-activity <?php echo ( ! $has_activity ) ? 'diy-forums--blank' : ''; ?>">

			<?php if ( $has_activity ) : ?>

				<div class="diy-block-header flex align-items-center">
					<div class="diy-block-header__title"><h3><?php _e( 'Activity', 'buddyboss-theme' ); ?></h3></div>
			...
			<?php else : ?>
				...
				<div class="diy-no-data diy-no-data--activity">
					<img class="diy-no-data__image" src="<?php echo get_template_directory_uri(); ?>/assets/images/svg/dfy-no-data-icon01.svg" alt="Activity" />
					<?php bp_nouveau_user_feedback( 'activity-loop-none' ); ?>
					<a href="<?php echo bp_get_activity_root_slug(); ?>" class="diy-no-data__link"><?php _e( 'Post an Update', 'buddyboss-theme' ); ?></a>
				</div>
