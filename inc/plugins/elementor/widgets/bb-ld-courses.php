...
class Ld_Courses extends Widget_Base {
	...
	protected function _register_controls() {
		...
		$course_cats = buddyboss_theme()->learndash_helper()->get_course_category( 'ld_course_category' );
		if( empty( $course_cats ) ){
			$course_cats = array();
		}
		$course_cats = array_column( $course_cats, 'name', 'term_id' );
		...
		$course_tags = buddyboss_theme()->learndash_helper()->get_course_category( 'ld_course_tag' );
		if( empty( $course_tags ) ){
			$course_tags = array();
		}
		$course_tags = array_column( $course_tags, 'name', 'term_id' );
		...
		<div id="learndash-content" <?php echo $this->get_render_attribute_string('ld-switch'); ?>>
			<form data-current_page_url="<?php echo esc_url( $current_page_url ); ?>" id="bb-courses-directory-form" class="bb-elementor-widget bb-courses-directory" method="get" action="">
				...
				<div class="grid-view bb-grid grid-box-<?php echo $course_box_border ; ?>">
					...
					<div id="course-dir-list" class="course-dir-list bs-dir-list columns-<?php echo $course_cols; ?> <?php echo ( $settings['switch_media'] ) ? 'course-dir-list--media' : 'course-dir-list--hidemedia'; ?> <?php echo ( $settings['switch_status'] ) ? 'course-dir-list--status' : 'course-dir-list--hidestatus'; ?>">
						<?php
						if ( $query->have_posts() ) {
							?>
							...
							<ul class="bb-card-list bb-course-items grid-view bb-grid <?php echo ( 'grid' === $view || $settings_skin == 'cover' ) ? '' : esc_attr( 'hide' ); ?>" aria-live="assertive" aria-relevant="all">
								<?php
								/* Start the Loop */
								while ( $query->have_posts() ) :
									$query->the_post();
	...
	private function get_active_plugin_category()
	{
		...
		return $archive_category_taxonomy;
